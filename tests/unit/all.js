...
describe("code snippet example", () => {
  ...
  describe("isEmpty", () => {
    ...
    it("_.isEmpty({a: '1'})", () => {
      assert.equal(_.isEmpty({ a: "1" }), isEmpty({ a: "1" }));
    });
  });
  describe("isInteger", () => {
    it("_.isInteger(3)", () => {
  ...
  describe("pull", () => {
    const pull = (arr, ...removeList) => {
      const removeSet = new Set(removeList);
      return arr.filter(el => {
        return !removeSet.has(el);
      });
    };
    it("pull([1,2,3,1,2,3], 2, 3) return [0, 1]", () => {
      const testArr = [1, 2, 3, 1, 2, 3];
      const testExample = pull(testArr, 2, 3);
      assert.deepStrictEqual(testExample, [0, 1]);
    });
  });
  ...
  describe("isUndefined", () => {
    ...
    it("_.isUndefined(undefinedVariable)", () => {
      assert.equal(
        _.isUndefined(undefinedVariable),
    ...
    it("_(undefinedVariable).isUndefined()", () => {
      assert.equal(
        _(undefinedVariable).isUndefined(),
        undefinedVariable === undefined
      );
    });
  });
